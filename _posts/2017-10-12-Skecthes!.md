...
With some toy experiments we were able to conclude that the following feature vector representation can be used to define the parts of a particular category. 
...
x_{1} & x_{2} & x_{3} & x_{4} \\
relative_area & roundedness & centroid(x) & centroid(y) \\
\end{array} \right)
$$
